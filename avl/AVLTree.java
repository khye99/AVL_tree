...
public class AVLTree<T extends Comparable<T>> {
	...
	private TreeNode<T> rightRotate(TreeNode<T> root) {
	    ...
	    TreeNode<T> z = root.parent;
	    
	   	root.setLeft(x.right);
	    x.setRight(root);
	    x.parent = z;
	    
	    updateHeight(root); // update the height on the original root node
	    updateHeight(x); // update height on the node that was shifted (aka rotated)
	    return x;
	}
